...
def main():
    ...
    parser.add_argument("--project_name", type=str, default="GRPO-Code-Edit")
    ...
    micro_batch_size = args.update_bsz // args.gradient_accumulation

    NNODES = 1
    MODEL_PATH = f"/YOUR_FOLDER/models/{model_name}"
    CKPTS_DIR = f"/YOUR_FOLDER/models/{project_name}/{exp_name}"
    TRAIN_FILE = "/YOUR_FOLDER/trainset.parquet"
    TEST_FILE = "/YOUR_FOLDER/testset.parquet"
    ...
    cmd = [
        ...
        f"actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu={micro_batch_size}",
        f"actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu={micro_batch_size}",
        f"actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu={micro_batch_size}",
